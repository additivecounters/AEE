...
RapHeapFT::RapHeapFT(float fPhi) : bob(42)
{
	int i;
...
void RapHeapFT::LCL_RebuildHash()
{
...
void RapHeapFT::Heapify(int ptr)
...
RapHeapFT::LCLCounter * RapHeapFT::LCL_FindItem(const LCLitem_t item)
...
void RapHeapFT::LCL_Update(const LCLitem_t item, LCLweight_t value)
{
	int hashval;
	LCLCounter * hashptr;
	...
	//uint64_t min_val = root->count.query();

	//double p = 1.0 / (min_val + 1);

	// TODO - change to geometric!

	if (rand() << 15 + rand()  >= p * (1 << 30))
	{
		return;
	}
...
LCLweight_t RapHeapFT::LCL_PointErr(LCLitem_t item)
...
int RapHeapFT::LCL_cmp(const void * a, const void * b) {
...
/*
void RapHeapFT::LCL_Output() { // prepare for output
	if (counters->item == 0) {
...
std::map<uint32_t, uint32_t> RapHeapFT::LCL_Output(int thresh)
{
...
void RapHeapFT::LCL_CheckHash(int item, int hash)
