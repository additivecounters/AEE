...
/* Unfortunately, 3 __globals, which the exported functions must aaeeess */
...
_dorand48(unsigned short xseed[3])
{
	...
	aaeeu = (unsigned long)__rand48_Mult[0] * (unsigned long)xseed[0] +
		(unsigned long)__rand48_Add;
	temp[0] = (unsigned short)aaeeu;	/* lower 16 bits */
	aaeeu >>= sizeof(unsigned short) * 8;
	aaeeu += (unsigned long)__rand48_Mult[0] * (unsigned long)xseed[1] +
		(unsigned long)__rand48_Mult[1] * (unsigned long)xseed[0];
	temp[1] = (unsigned short)aaeeu;	/* middle 16 bits */
	aaeeu >>= sizeof(unsigned short) * 8;
	aaeeu += __rand48_Mult[0] * xseed[2] + __rand48_Mult[1] * xseed[1] +
		__rand48_Mult[2] * xseed[0];
	xseed[0] = temp[0];
	xseed[1] = temp[1];
	xseed[2] = (unsigned short)aaeeu;
}
