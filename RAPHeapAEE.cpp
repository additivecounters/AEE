...
RAPHeapAEE::RAPHeapAEE(float fPhi, int seed) : bob(seed)
{
...
RAPHeapAEE::~RAPHeapAEE()
{
	free(hashtable);
	free(counters);
}

int RAPHeapAEE::hash_item(LCLitem_t item)
{
	return (int)hash31(hasha, hashb, item) % hashsize;
}


void RAPHeapAEE::LCL_RebuildHash()
{
...
void RAPHeapAEE::Heapify(int ptr)
...
RAPHeapAEE::LCLCounter * RAPHeapAEE::LCL_FindItem(LCLitem_t item)
...
void RAPHeapAEE::increment_and_cond_divide(LCLCounter* hashptr)
{
...
void RAPHeapAEE::Probabilistic_Increment(const FTitem_t ft_item, LCLweight_t value)
{
...
void RAPHeapAEE::LCL_Update(const FTitem_t ft_item, LCLweight_t value)
{
	...
	min_val = root->count.return_actual_val();
	p = 1.0 / (min_val + 1);

}

int RAPHeapAEE::LCL_Size()
{ // return the size of the data structure in bytes
	return sizeof(this) + (hashsize * sizeof(int)) +
		(size * sizeof(LCLCounter));
}

LCLweight_t RAPHeapAEE::LCL_PointEst(const FTitem_t ft_item)
...
/*
LCLweight_t RAPHeapAEE::LCL_PointErr(const FTitem_t ft_item)
...
int RAPHeapAEE::LCL_cmp(const void * a, const void * b) {
...
void RAPHeapAEE::LCL_Output() { // prepare for output
	if (counters->item == 0) {
...
std::map<uint32_t, uint32_t> RAPHeapAEE::LCL_Output(int thresh)
{
...
void RAPHeapAEE::LCL_CheckHash(int item, int hash)
